...
class CnnPolicy(object):
    ...
    def __init__(self, sess, ob_space, ac_space, noptions, nbatch, nsteps, reuse=False): #pylint: disable=W0613
        ...
        with tf.variable_scope("model", reuse=reuse):
            ...
            intra_pi_fc = fc(h, 'pi', nact*noptions, init_scale=0.01)
            intra_pi = tf.nn.softmax(tf.reshape(intra_pi_fc, (nbatch, noptions, nact)))
            # to gather action probabilities only for selected options!
            gather_indices = tf.range(nbatch) * noptions + opt
            opt_pi = tf.gather(tf.reshape(intra_pi, (nbatch*noptions, nact)), gather_indices, axis=0)
            # option termination
            betas = tf.nn.softmax(fc(h, 'b', noptions, init_scale=0.01))
            beta = tf.gather(tf.reshape(betas, [-1]), gather_indices, axis=0)

        # self.pdtypes = [make_pdtype(ac_space) for _ in range(noptions)]
        # self.pd = [self.pdtypes[i].pdfromflat(pi) for i in range(noptions)]
        ...
        def step(ob, opt, *_args, **_kwargs):
            a, beta, opt_done = sess.run([a0, beta, beta0], {X:ob, opt:opt})
            return a, beta, neglogp
